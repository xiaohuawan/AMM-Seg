...
# class PostProcess:
#     def __init__(self, data_url, json_url, upload_url, download_path="downloaded_images") -> None:
#         self.data_url = data_url
#         self.json_url = json_url
#         self.upload_url = upload_url  # 上传结果的URL
...
#             logger.error(f"Failed to download JSON from {self.json_url}: {e}")
#             return []
...
#     def upload_processed_images(self, directory):
#         """
#         上传处理后的图像到服务器
#         """
...
#                 except requests.RequestException as e:
#                     logger.error(f"Failed to upload {file_path}. Status code: {response.status_code}, Error: {e}")

#     def del_dot_byOpen(self, kernel_size: int, path_list):
#         """
#         对分割的结果进行开运算, 去除小点
#         """
#         save_path = os.path.join(self.download_path, f"processed_opening_{kernel_size}")
...
#         # 上传处理后的图像
#         self.upload_processed_images(save_path)

#     def del_dot_byArea(self, min_area: int, path_list):
#         save_path = os.path.join(self.download_path, f"processed_area_{min_area}")
#         os.makedirs(save_path, exist_ok=True)
...
#     upload_url = "http://localhost:3000/free/seg/postP"

#     # 设置处理类型和参数
#     process_type = "opening"  # "opening" 或 "area"
#     param = 2  # 对于 "opening"，参数范围应在[2,8]，步长为1；对于 "area"，参数范围应在[10,100]，步长为10

#     # 初始化PostProcess实例
#     process = PostProcess(data_url=data_url, json_url=json_url, upload_url=upload_url)
...
#             save_path2 = os.path.abspath(os.path.join(current_path, 'AMM-Seg', 'vite', 'public', f"postP_{current_time}")) 
...
#     save_path = os.path.abspath(os.path.join(current_path, 'AMM-Seg', 'vite', 'public', 'postP')) 
#     # 设置处理类型和参数
#     process_type = "opening"  # "opening" 或 "area"
